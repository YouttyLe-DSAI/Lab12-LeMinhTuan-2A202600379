...
- Stage 1 làm gì?
- ans: Cài đặt gcc, libpq-dev và pip install tất cả dependencies vào /root/.local
- Stage 2 làm gì?
- ans: Bắt đầu từ image sạch python:3.11-slim, chỉ copy thư mục /root/.local từ Stage 1 sang — không copy pip, gcc, hay build tools
- Tại sao image nhỏ hơn?
- ans: Stage 2 không chứa compiler, build tools (gcc ~200MB). Chỉ giữ lại những gì cần để chạy app, không cần để build. Kết quả: từ ~1.6GB xuống còn ~160-200MB
Build và so sánh:
```bash
...
graph TD
    ...
    Nginx["🔀 Nginx<br/>Port 80/443<br/>Reverse Proxy"] --> Agent

    subgraph Docker Internal Network
        Agent["🤖 Agent FastAPI<br/>Port 8000<br/>production-agent"] --> Redis
        Agent --> Qdrant
        Redis["⚡ Redis<br/>Port 6379<br/>Cache & Rate Limit"]
        Qdrant["🗄️ Qdrant<br/>Port 6333<br/>Vector Database"]
    end
```
...
6. Get public URL: 
https://lecture-day12-production.up.railway.app
```bash
railway domain
```
...
(Vin20k) D:\FPTU\Vin\Lecture-Day-12\03-cloud-deployment\railway>curl https://lecture-day12-production.up.railway.app/health
{"status":"ok","uptime_seconds":153.4,"platform":"Railway","timestamp":"2026-04-17T07:05:25.831194+00:00"}
(Vin20k) D:\FPTU\Vin\Lecture-Day-12\03-cloud-deployment\railway>
...
More?   -H "X-API-Key: Vinuni-secret" ^
More?   -d "{\"question\": \"Tell me a funny joke about programming in 1 sentence.\"}"
{"question":"Tell me a funny joke about programming in 1 sentence.","answer":"Why do programmers prefer dark mode? Because light attracts bugs!","platform":"Railway","llm_type":"OpenAI"}
```
